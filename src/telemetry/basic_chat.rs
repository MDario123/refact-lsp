...
pub async fn compress_basic_chat_telemetry_to_file(
    cx: Arc<ARwLock<global_context::GlobalContext>>,
) {
    let mut key2cnt = HashMap::new();
    let mut key2dict = HashMap::new();

    for rec in cx.read().await.telemetry.read().unwrap().tele_chat.iter() {
        let key = format!("{}/{}/{}", rec.scope, rec.success, rec.error_message);
